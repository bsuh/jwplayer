package org.mangui.jwplayer.media {
    ...
    public class HLSProvider extends MediaProvider {
        ...
        protected function _fragmentHandler(event : HLSEvent) : void {
            _level = event.loadMetrics.level;
            sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_META, {metadata:{bandwidth:Math.round(event.loadMetrics.bandwidth / 1024), droppedFrames:_hls.stream.info.droppedFrames, currentLevel:(_level + 1) + ' of ' + _levels.length + ' (' + Math.round(_levels[_level].bitrate / 1024) + 'kbps, ' + _levels[_level].width + 'px)', width:_videoWidth}});
        };
